"""Manage recurring tasks with SQLite-backed persistence."""
...
class ScheduleManager:
    """High level interface for scheduled jobs using :mod:`schedule`."""
    ...
    def __init__(self, db_path: str = "data/schedules.db") -> None:
        ...
        ----------
        db_path:
            Path to the SQLite database for persisting tasks.
        """
    ...
    def _persist_task(self, name: str, func: Callable[..., object], interval: int) -> None:
    ...
    def get_schedule(self, name: str) -> Optional[Dict[str, int]]:
        """Return the schedule record for ``name`` if it exists."""
    ...
    def update_schedule(self, name: str, interval: int) -> bool:
        """Update the interval for ``name`` and reschedule if running."""
        with self.conn:
            cur = self.conn.execute(
    ...
    def delete_schedule(self, name: str) -> bool:
        """Delete the schedule entry for ``name``."""
        with self.conn:
            cur = self.conn.execute(
        ...
        if deleted and name in self.jobs:
    ...
    # Task scheduling API
    # ------------------------------------------------------------------
    def add_task(self, name: str, func: Callable[..., object], interval: int) -> schedule.Job:
    ...
    def remove_task(self, name: str) -> bool:
        """Cancel a scheduled job and remove its record."""
    ...
    def list_tasks(self) -> Dict[str, schedule.Job]:
        """Return a mapping of task names to their jobs."""
        return dict(self.jobs)

    def run_pending(self) -> None:
        """Execute any jobs that are scheduled to run."""
